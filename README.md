...
- **PDF import** — copies the file into app storage, reads a text layer when one exists, then opens a four-slide review: recipe details → ingredients → **ingredients-or-notes cut** (CUT bar / arrows / tap a line) → directions & notes. Dual metric on an ingredient line is dropped when a cup/spoon unit is present (`2 cups (250g) flour` → `2 cups: flour`). Parenthetical notes stay on that same ingredient (`1 onion (diced)` is one line). Yield / Serves / Makes fill servings; volume yields such as `Yield: 2 cups` stay out of the ingredient list (the number goes to servings, the yield phrase to Notes). Prep / Cook / Total times fill when labeled. If extract fails, the PDF is still attached and you fill the form.
- **Recipe detail** — optional hero photo, **Edit** (opens the same form as create/review and updates that recipe), **US / Grams** toggle (Grams converts cups, tablespoons, and teaspoons using typical ingredient densities; `~` marks an approximate weight), Prep / Cook / Total chips when set, servings + batch scale (− / N× / +), tappable ingredient checklist (`2 cups: all-purpose flour`), numbered directions. PDF recipes can reopen the attached file.
