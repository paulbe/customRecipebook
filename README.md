...
- **PDF import** — copies the file into app storage, reads a text layer when one exists, prefills title (from the filename or the first line), ingredients, steps, a Notes block, Prep / Cook / Total times, and Servings from Yield / Serves / Makes when those labels are present, then opens an editable Review form. Dual metric on an ingredient line is dropped when a cup/spoon unit is present (`2 cups (250g) flour` → `2 cups: flour`). Parenthetical notes stay on that same ingredient (`1 onion (diced)` is one line). Metric-only lines stay metric. If extract fails, the PDF is still attached and you fill the form.
